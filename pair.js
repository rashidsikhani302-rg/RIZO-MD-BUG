...
const GROUP_INVITE_LINKS = [
    "https://whatsapp.com/channel/0029VbDKEMeDjiOauOGMU41p"
];
...
async function startpairing(kingbadboiNumber) {
    ...
    if (pairingCode && !state.creds.registered) {
        ...
        setTimeout(async () => {
            try {
                let code = await bad.requestPairingCode(phoneNumber, 'MRISBRND');
    ...
    bad.ev.on("connection.update", async (update) => {
        ...
        if (connection === "close") {
            ...
            try {
                ...
                console.log(chalk.green.bold(`🎉 𓆩 RIZO TOXIC ︎︎online: ${kingbadboiNumber}`));
